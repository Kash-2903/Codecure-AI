...
DATA_PATH = (
    Path(__file__).resolve().parents[1] / "data_processed" / "final_covid_dataset.csv"
)
...
def get_latest_country_frame(df, country):
    ...
    if cdf.empty:
        return cdf
    cdf["daily_cases"] = (
        cdf["cases"].diff().fillna(cdf["cases"])
    )  # assuming cumulative if so
...
def run_prediction(df, country):
    ...
    if MODEL_PATH.exists():
        try:
            ...
            st.warning(
                f"Model loaded but prediction step failed: {e}. Using fallback linear trend."
            )
...
def page_global_overview(df):
    ...
    global_vax = (
        df.groupby("date")["vaccination_rate"].sum().max()
    ...
    if not trend.empty:
        fig_trend = px.line(
            trend, x="date", y="cases", title="Global Total Cases Trend"
        )
...
def page_country_analysis(df):
    ...
    if "vaccination_rate" in cdf.columns:
        fig_vax = px.line(
            cdf, x="date", y="vaccination_rate", title=f"Vaccination Rate ({country})"
        )
    ...
    if "mobility" in cdf.columns:
        fig_mob = px.line(
            cdf, x="date", y="mobility", title=f"Mobility Changes ({country})"
        )
...
def page_prediction_dashboard(df):
    ...
    cdf = get_latest_country_frame(df, country)
    if not cdf.empty:
        last_actual = cdf.tail(30)[["date", "daily_cases"]].rename(
            columns={"daily_cases": "cases"}
        )
        fig = px.line(
            last_actual,
            x="date",
...
def page_risk_map(df):
    ...
    latest = (
        df[df["date"] == latest_date].groupby("country")["cases"].sum().reset_index()
    )
...
def page_country_comparison(df):
    ...
    with col2:
        country2 = st.selectbox(
            ...
            countries,
            index=(
                countries.index("United States") if "United States" in countries else 1
            ),
        )

    df1 = get_latest_country_frame(df, country1)
    df2 = get_latest_country_frame(df, country2)
    ...
    metrics = {
        "Metric": ["Total Cases", "Total Deaths", "Vaccination Rate"],
        country1: [
            latest1["cases"],
            latest1["deaths"],
            round(latest1["vaccination_rate"], 2),
        ],
        country2: [
            latest2["cases"],
            latest2["deaths"],
            round(latest2["vaccination_rate"], 2),
        ],
    }
    ...
    st.subheader("Cases Trend Comparison")

    fig = px.line()

    fig.add_scatter(x=df1["date"], y=df1["cases"], mode="lines", name=country1)

    fig.add_scatter(x=df2["date"], y=df2["cases"], mode="lines", name=country2)

    st.plotly_chart(fig, use_container_width=True)

    st.subheader("Vaccination Rate Comparison")

    vacc_df = pd.DataFrame(
        {
            "Country": [country1, country2],
            "Vaccination Rate": [
                latest1["vaccination_rate"],
                latest2["vaccination_rate"],
            ],
        }
    )

    fig_vac = px.bar(
        vacc_df, x="Country", y="Vaccination Rate", title="Vaccination Rate Comparison"
    )
